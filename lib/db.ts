...
export async function getAllUsers() {
  const sql = "SELECT id, name, email, role, department, created_at FROM users ORDER BY created_at DESC"
...
export async function createLeaveRequest(data: {
  ...
  const sql = `
    ...
    VALUES (?, ?, ?, ?, ?, ?, 'pending')
  `
  const params = [data.user_id, data.jenis_cuti_id, data.start_date, data.end_date, data.total_days, data.reason]
  return await query(sql, params)
}

export async function updateLeaveRequestStatus(id: number, status: string, reviewedBy?: number, reviewNotes?: string) {
  const sql = `
...
export async function createUserQuota(data: {
  ...
  const sql = `
    ...
    VALUES (?, ?, ?, ?)
  `
  return await query(sql, [data.user_id, data.year, data.total_quota, data.used_quota || 0])
}

export async function updateUserQuota(userId: number, year: number, usedQuota: number) {
  const sql = "UPDATE kuota_cuti SET used_quota = ? WHERE user_id = ? AND year = ?"
  return await query(sql, [usedQuota, userId, year])
}

// Dashboard statistics
export async function getDashboardStats() {
  const totalEmployees = await query('SELECT COUNT(*) as count FROM users WHERE role = "employee"')
  const pendingRequests = await query('SELECT COUNT(*) as count FROM leave_requests WHERE status = "pending"')
  const approvedRequests = await query('SELECT COUNT(*) as count FROM leave_requests WHERE status = "approved"')
  const rejectedRequests = await query('SELECT COUNT(*) as count FROM leave_requests WHERE status = "rejected"')

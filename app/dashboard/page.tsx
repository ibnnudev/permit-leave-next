...
import { getCutiByUserId, getCutiKuotaByUser, getCutiStats } from "@/lib/db"
...
export default async function EmployeeDashboard() {
  ...
  const [cutiList, kuotaList, stats] = await Promise.all([
  ...
  const userStats = {
    pending_requests: cutiList.filter((c) => c.status === "pending").length,
    dalam_proses_requests: cutiList.filter((c) => c.status === "dalam_proses").length,
    approved_requests: cutiList.filter((c) => c.status === "disetujui").length,
    total_days_taken: cutiList
      .filter((c) => c.status === "disetujui")
      .reduce((sum, c) => sum + (c.days_requested || 0), 0),
  }
        ...
        <div className="px-4 py-6 sm:px-0">
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              {user.jabatan} • {user.lembaga_nama}
            </p>
          </div>
                  ...
                  {kuotaList.length > 0 ? (
                    kuotaList.map((kuota) => (
                      <div key={kuota.id} className="flex justify-between items-center p-3 border rounded-lg">
                        <div>
                          <p className="font-medium">{kuota.jenis_cuti_nama}</p>
                          <p className="text-sm text-gray-600">Kuota tahunan</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-lg">{kuota.jatah_total - kuota.jatah_terpakai}</p>
